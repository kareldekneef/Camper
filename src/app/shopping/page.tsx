...
// Shopping category detection (same logic as trip detail)
...
interface AggregatedItem {
  ...
  items: TripItem[]; // Original trip items
  tripNames: string[];
  allPurchased: boolean;
  allPacked: boolean;
  anyPurchased: boolean;
  isShoppingCategory: boolean;
}
...
export default function ShoppingPage() {
  ...
  // Aggregate items across trips (group by name + categoryId)
  const aggregatedItems = useMemo(() => {
    const activeItems = tripItems.filter((ti) => activeTripIds.has(ti.tripId));
    const groupMap = new Map<string, AggregatedItem>();

    for (const item of activeItems) {
      const key = `${item.name.toLowerCase().trim()}|${item.categoryId}`;
      const existing = groupMap.get(key);
      const isShopping = shoppingCategoryIds.has(item.categoryId);
  ...
  // Filter items: only show items that still need action
  const filteredItems = useMemo(() => {
    ...
    if (filter === 'todo') {
      items = items.filter((item) => {
        if (item.isShoppingCategory) {
          // Shopping items: show if not all packed
          return !item.allPacked;
        }
        // Other items: show if not all checked
        return !item.allPacked;
      });
    } else if (filter === 'purchased') {
      // Show only purchased but not yet packed
  ...
  const groupedByCategory = useMemo(() => {
    ...
    for (const [catId, items] of map) {
      map.set(catId, items.sort((a, b) => a.name.localeCompare(b.name, 'nl')));
    }
    return map;
  }, [filteredItems]);

  const sortedCategories = categories
    .filter((c) => groupedByCategory.has(c.id))
    .sort((a, b) => {
      // Shopping category first
      const aIsShopping = isShoppingCategory(a.name) ? 0 : 1;
      const bIsShopping = isShoppingCategory(b.name) ? 0 : 1;
      if (aIsShopping !== bIsShopping) return aIsShopping - bIsShopping;
  ...
  // Stats
  const totalTodo = aggregatedItems.filter((i) => !i.allPacked).length;
  const totalPurchased = aggregatedItems.filter((i) => i.anyPurchased && !i.allPacked).length;
    ...
    <div className="mx-auto max-w-lg px-4 pt-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Overzicht van {activeTrips.length} actieve trip{activeTrips.length !== 1 ? 's' : ''}
        </p>
      </div>
      ...
      <div className="space-y-2 pb-4">
        {sortedCategories.map((category) => {
          ...
          const isShopping = isShoppingCategory(category.name);
          const isCollapsed = collapsedCategories.has(category.id);
          const catPurchased = items.filter((i) => i.allPurchased).length;
          const catPacked = items.filter((i) => i.allPacked).length;
...
function ShoppingItemRow({
  ...
  // Regular items: just show checked/unchecked
  return (
    <div className="px-3 py-2.5">
      <div className="flex items-center gap-2">
        <button onClick={onTogglePacked} className="shrink-0">
          {item.allPacked ? (
